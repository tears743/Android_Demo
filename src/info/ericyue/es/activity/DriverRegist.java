...
public class DriverRegist extends Activity {
	
	public static void launch (Context c , Bundle bundle){
		Intent intent = new Intent(c,DriverRegist.class);
	...
	info.ericyue.es.myView.SelectAddressView driverRegistAddress;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		driverRegistAddress = (SelectAddressView)findViewById(R.id.cargoregistaddress);
...
//		district = (Button)findViewById(R.id.odistrict);
	...
	public void CheckInput(){
		if(!CheckUserName(userName.getText().toString())){
			Toast.makeText(DriverRegist.this, "用户名只能由4-18个字母或数字组成（不区分大小写），不能用中文，注册成功后不可修改",Toast.LENGTH_LONG).show();
		}
			else if(!CheckPassword(passWord1.getText().toString())){
			Toast.makeText(DriverRegist.this, "密码只能由6-16个字母或数字组成（区分大小写），不能用中文",Toast.LENGTH_LONG).show();
		}
				else if(!passWord1.getText().toString().equals(passWord2.getText().toString())){
				Toast.makeText(DriverRegist.this, "两次密码不一致！",Toast.LENGTH_LONG).show();
			}
					else{
						String state="";
						try{
							...
							if(state.contains("0"))
							{
								Toast.makeText(DriverRegist.this, "用户名有重复，请更改。",Toast.LENGTH_LONG).show();
							}
							else
							{	
								Toast.makeText(DriverRegist.this, "注册成功"+state,Toast.LENGTH_LONG).show();
								Bundle bundle = new Bundle();
								LoginActivity.launch(DriverRegist.this, bundle);
								DriverRegist.this.finish();
							}
					}
	}
	...
	private void GetRegistInfo() {
		...
		address=driverRegistAddress.getFromAddress();
