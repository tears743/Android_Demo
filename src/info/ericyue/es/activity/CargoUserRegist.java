...
public class CargoUserRegist extends Activity {
	
	public static void launch (Context c , Bundle bundle){
		Intent intent = new Intent(c, CargoUserRegist.class);
	...
	private Button submit;
	 ...
	 info.ericyue.es.myView.SelectAddressView cargoRgistAddress;
...
//		requestWindowFeature(Window.FEATURE_CUSTOM_TITLE);
		...
		cargoRgistAddress = (SelectAddressView)findViewById(R.id.cargoregistaddress);
	...
	public void CheckInput(){
		if(!CheckUserName(userName.getText().toString())){
			Toast.makeText(CargoUserRegist.this, "用户名只能由4-18个字母或数字组成（不区分大小写），不能用中文，注册成功后不可修改",Toast.LENGTH_LONG).show();
		}
			else if(!CheckPassword(passWord1.getText().toString())){
			Toast.makeText(CargoUserRegist.this, "密码只能由6-16个字母或数字组成（区分大小写），不能用中文",Toast.LENGTH_LONG).show();
		}
				else if(!passWord1.getText().toString().equals(passWord2.getText().toString())){
				Toast.makeText(CargoUserRegist.this, "两次密码不一致！",Toast.LENGTH_LONG).show();
			}
					else{
						String state="";
						try{
							...
							if(state.contains("0"))
							{
								Toast.makeText(CargoUserRegist.this, "用户名有重复，请更改。",Toast.LENGTH_LONG).show();
							}
							else
							{	
								Toast.makeText(CargoUserRegist.this, "注册成功"+state,Toast.LENGTH_LONG).show();
								Bundle bundle = new Bundle();
								LoginActivity.launch(CargoUserRegist.this, bundle);
								CargoUserRegist.this.finish();
							}
					}
	}
	...
	private void GetRegistInfo() {
		...
		address=cargoRgistAddress.getFromAddress();
